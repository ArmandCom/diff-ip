...
class QuerierModel(nn.Module):
    ...
    def __init__(
        ...
        self.out = nn.Sequential(
            ...
            conv_nd(dims, ch, out_channels, 1), # zero_module() (why 0 module)
            nn.Sigmoid()
        )
    ...
    def forward(self, x, y=None, masks=None, layer_idx=None):
        """
        Apply the model to an input batch.
        ...
        ## Note: For later conditioning the querier to previous queries.
        # if self.num_classes:
        #     # assert y.shape == (x.shape[0],)   # assert for discrete class labels
        #     if self.dataset == 'clevr_pos' or self.dataset == 'clevr_pos_multiple':
        #         label_emb = th.empty((y.shape[0], self.time_embed_dim), device=y.device)
        ...
        # TODO: First approach: Coordinate regression. NOT selection.
        h = x.type(self.dtype)
        for module in self.input_blocks:
            h = module(h, emb)
            hs.append(h)
        ...
        #     h = module(h, emb)
        h = h.type(x.dtype)
        h = self.out(h).squeeze(-1).squeeze(-1)
        return h
