...
from arch.modules import UNet_conditional, UNet_conditional_cifar10, EMA
...
def parseargs():
    ...
    parser.add_argument('--run_name', type=str, default='debug_matrix_cifar', help='save directory')
...
def main(args):
    ...
    trainset = datasets.CIFAR10(args.data_dir, train=True, transform=transform, download=True)
    testset = datasets.CIFAR10(args.data_dir, train=False, transform=transform, download=True)
    ...
    querier = QuerierMNIST(num_classes=QUERY_ALL, tau=args.tau_start,  q_size=(QH, QW), in_channels=C)
    ...
    for epoch in range(args.epochs):
        ...
        for train_i, (train_images, train_labels) in enumerate(tqdm(trainloader)):
            ...
            if (train_i % 1000 == 0 or epoch == args.epochs - 1) and args.sample_all:

                unet.eval()
                ema_unet.eval()
                querier.eval()
                sample_model = ema_unet
                sample_labels = [0,1,7,9]
                args.num_viz = len(sample_labels)
                test_images_list = []
                for test_i, (test_images, test_labels) in enumerate(tqdm(testloader)):
                    if test_i < 3:
                        continue
