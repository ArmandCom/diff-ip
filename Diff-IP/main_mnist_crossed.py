...
def parseargs():
    ...
    parser.add_argument('--ckpt_diff', type=str, default=None, help='load checkpoint')
    ...
    parser.add_argument('--run_name', type=str, default='mnist_diff_mix', help='save directory')
...
def main(args):
    ...
    ## Load checkpoint
    if args.ckpt_diff is not None and args.ckpt_quer is not None:
        ckpt_dict_diff = torch.load(args.ckpt_diff, map_location='cpu')
        ckpt_dict_quer = torch.load(args.ckpt_quer, map_location='cpu')
        unet.load_state_dict(ckpt_dict_diff['unet'])
        ema_unet.load_state_dict(ckpt_dict_diff['ema_unet'])
        querier.load_state_dict(ckpt_dict_quer['querier'])
        optimizer.load_state_dict(ckpt_dict_diff['optimizer'])
        scheduler.load_state_dict(ckpt_dict_diff['scheduler'])
    ...
    for epoch in range(args.epochs):
        ...
        for train_i, (train_images, train_labels) in enumerate(tqdm(trainloader)):
            ...
            if ((train_i % 1000 == 0 or epoch == args.epochs - 1) or args.sample_all) and evaluation:
                    ...
                    for i in range(args.max_queries_test):
                        with torch.no_grad():
                            ...
                            if (i == 0 or i == args.max_queries_test - 1 or args.sample_all) :
                                ...
                                utils.save_images(sampled_images, os.path.join(args.save_dir, args.run_name, f"{pref}{epoch}-nq{i}.png"), nrow=args.num_viz)
                                if i == args.max_queries_test - 1:
                                    ...
                                    utils.save_images(sampled_images, os.path.join(args.save_dir, args.run_name, f"{pref}{epoch}-nq{i}_ema.png"), nrow=args.num_viz)
                                gt_plot = (test_inputs.clamp(-1, 1) + 1) / 2
                                gt_plot = torch.repeat_interleave((gt_plot * 255).type(torch.uint8), args.num_viz, 0)
                                utils.save_images(gt_plot, os.path.join(args.save_dir, args.run_name, f"{pref}{epoch}-nq{i}-qs.png"), nrow=args.num_viz)
